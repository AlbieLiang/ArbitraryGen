 ...
 */
public class PsychicProcessor implements ArbitraryGenProcessor {

    private static final String TAG = "AG.PsychicProcessor";
        ...
        for (int t = 0; t < taskArray.size(); t++) {
            ...
            if (dependsOnArray != null && !dependsOnArray.isEmpty()) {
                ...
                for (int i = 0; i < dependsOnArray.size(); i++) {
                    JSONObject dependsOnInfo = dependsOnArray.optJSONObject(i);
            ...
            JSONObject result = context.execProcess(context.getAllProcessor(), processor, taskInfo);
            if (result == null) {
                Log.d(TAG, "execute task processor(%s) result is null, switch to next task.", processor);
                continue;
            }
            ...
            if (resultToArray != null && !resultToArray.isEmpty()) {
                for (int i = 0; i < resultToArray.size(); i++) {
                    JSONObject resultToInfo = resultToArray.optJSONObject(i);
                    ...
                    JSONObject r = context.execProcess(context.getAllProcessor(), p, resultToInfo);
